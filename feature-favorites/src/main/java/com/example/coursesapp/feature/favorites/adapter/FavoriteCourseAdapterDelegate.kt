...
import com.example.coursesapp.feature.favorites.databinding.ItemFavoriteCourseBinding
import com.hannesdorfmann.adapterdelegates4.dsl.adapterDelegateViewBinding

fun favoriteCourseAdapterDelegate(
    onFavoriteClick: (Int) -> Unit
) = adapterDelegateViewBinding<Course, Course, ItemFavoriteCourseBinding>(
    { layoutInflater, parent ->
        ItemFavoriteCourseBinding.inflate(layoutInflater, parent, false)
    }
) {
    ...
    bind {
        ...
        binding.bookmarkIcon.setImageResource(CoreR.drawable.ic_bookmark_filled)
        binding.bookmarkIcon.setColorFilter(
            ContextCompat.getColor(context, CoreR.color.primary_green)
        )
    }
}
